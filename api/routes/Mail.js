...
router.get("/newmail/:id", async (req, res) => {
  const myId = req.params.id;
  const newmail = await Message.findOne({
...
router.get("/newmailcount/:id", async (req, res) => {
  const myId = req.params.id;
  const newmails = await Message.findAll({
    where: { recipientId: myId, read: false },
  });
...
router.get("/auctions/:id", validateTheToken, async (req, res) => {
  ...
  const mails = await Mail.findAll({
    where: {
      [Op.or]: [
        { [Op.and]: [{ sellerId: bidderId }, { bidderId: myId }] },
        { [Op.and]: [{ sellerId: myId }, { bidderId: bidderId }] },
      ],
    },
  });
...
router.get("/inbox/:id", validateTheToken, async (req, res) => {
  ...
  const inboxMessages = await Message.findAll({
    where: { recipientId: myId, deletedInbox: false },
  });
...
router.get("/outbox/:id", validateTheToken, async (req, res) => {
  ...
  const inboxMessages = await Message.findAll({
    where: { senderId: myId, deletedOutbox: false },
  });
...
router.get("/contacts/:id", validateTheToken, async (req, res) => {
  ...
  const contacts = await Contact.findAll({ where: { UserId: myId } });
