...
router.get("/", async (req, res) => {
  ...
  for (var i = 0; i < allItems.length; i++) {
    if (allItems[i].number_of_bids > 0) {
      ...
      const contactalready = await Contact.findOne({
        where: { contactId: userito.id, UserId: biderito.id },
      });
...
function myChildren(categoriesList, superCategoryId) {
  ...
  for (var i = 0; i < categoriesList.length; i++) {
    if (categoriesList[i].CategoryId == superCategoryId) {
      categs.push.apply(
        categs,
        myChildren(categoriesList, categoriesList[i].id)
      );
    }
  }
  return categs;
...
router.get("/categories/:id", async (req, res) => {
  ...
  for (var i = 0; i < categs.length; i++) {
    const freshItems = await Item.findAll({
      where: { furthermostCategoryId: categs[i], state: "AVAILABLE" },
    });
...
router.get("/fetchy/:id", async (req, res) => {
  ...
  if (req.body && req.body.userId) {
    ...
    var UserData = await User.findOne({
      where: { UserId: userId, ItemId: myId },
    });
...
router.put("/purchase/:id", validateTheToken, async (req, res) => {
  ...
  // looking for one is enough since its symmetric
  const contactalready = await Contact.findOne({
    where: { contactId: sellerId, UserId: bidderId },
  });
  ...
  if (username == "admin") {
    ...
    for (var i = 0; i < allItems.length; i++) {
      ...
      categories.push.apply(
        categories,
        myParents(allCategories, allItems[i].furthermostCategoryId)
      );
  ...
  if (username == "admin") {
    ...
    for (var i = 0; i < allItems.length; i++) {
      ...
      if (allItems[i].latitudeLongitude) {
        ...
        itemsXMLformat +=
          allItems[i].latitudeLongitude.coordinates[0].toString();
        itemsXMLformat += '" Longitude="';
        itemsXMLformat +=
          allItems[i].latitudeLongitude.coordinates[1].toString();
        itemsXMLformat += '">';
      } else {
        itemsXMLformat += "\t\t<Location>";
      }
...
router.post("/importxmls", async (req, res) => {
  ...
  // one by one
  await fs.readFile(
    path.join(__dirname, "..", "ebayXMLs", "items-23.xml"),
    async (err, data) => {
      await parser.parseString(data, async (err, result) => {
        if (err) {
          throw err;
        ...
        for (var p = 0; p < result.Items.Item.length; p++) {
          ...
          if (parseInt(itemito.Number_of_Bids[0]) == 0) {
            ...
            const activeUsers = await User.findAll({
              where: { approved: true, admin: false },
            });
            ...
            if (itemito.Location[0]._) {
              ...
              var point = {
                type: "Point",
                coordinates: [
                  itemito.Location[0].$.Latitude,
                  itemito.Location[0].$.Longitude,
                ],
              };
            ...
            for (i = 0; i < parseInt(itemito.Number_of_Bids[0]); i++) {
              ...
              const pickBuyer =
                activeBuyers[Math.floor(Math.random() * activeBuyers.length)];

              var bid = {
                time: itemito.Bids[0].Bid[i].Time[0],
                amount: parseFloat(
                  itemito.Bids[0].Bid[i].Amount[0].substring(1)
                ),
                ItemId: newItem.id,
              };
