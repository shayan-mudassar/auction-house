...
export default function MailInbox() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://localhost:33123/mail/inbox/${authState.id}`, head)
      .then((res) => {
  ...
  const displayMessage = (name) => {
    if (name.read === false) {
      axios
        .put(`https://localhost:33123/mail/message/read/${name.id}`)
        .then((res) => {});
  ...
  const deleteMessage = (name) => {
    axios
      .put(`https://localhost:33123/mail/message/deleteinbox/${name.id}`)
      .then((res) => {});
    let mails = [...mailList];
  ...
  return (
    <>
      <Grid container spacing={1}>
        ...
        <Grid item xs={7.6}>
          {clicked && <MessageCard message={messageDisplayed} type="inbox" />}
        </Grid>
      </Grid>
    </>
  );
