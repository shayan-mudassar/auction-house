...
export default function MailMessage() {
  ...
  const messageSend = () => {
    if (Object.keys(personContact).length === 0) {
      ...
      axios
        .post(
          `https://localhost:33123/mail/message/${authState.id}`,
          body,
          head
        )
        .then((res) => {
  ...
  const handleChange = (event) => {
    ...
    axios
      .get(`https://localhost:33123/mail/auctions/${authState.id}`, head)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://localhost:33123/mail/contacts/${authState.id}`, head)
      .then((res) => {
        console.log(res.data);
        setMails(res.data);
      });
              ...
              <FormControl sx={{ m: 4, width: 320 }}>
                ...
                >
                  {mails.map((name) => (
                    ...
                    >
                      {name.contactUsername} ({name.contactSurname},{" "}
                      {name.contactName})
                    </MenuItem>
                  ))}
                </Select>
