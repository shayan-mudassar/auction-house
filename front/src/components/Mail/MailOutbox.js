...
export default function MailOutbox() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://localhost:33123/mail/outbox/${authState.id}`, head)
      .then((res) => {
  ...
  const displayMessage = (name) => {
    axios
      .get(`https://localhost:33123/auth/fetchy/${name.senderId}`)
      .then((respo) => {
  ...
  const deleteMessage = (name) => {
    axios
      .put(`https://localhost:33123/mail/message/deleteoutbox/${name.id}`)
      .then((res) => {});
    let mails = [...mailList];
  ...
  return (
    <Grid container spacing={1}>
      ...
      <Grid item xs={7.6}>
        {clicked && <MessageCard message={messageDisplayed} type="outbox" />}
      </Grid>
    </Grid>
  );
}
