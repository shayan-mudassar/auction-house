...
function Users() {
  ...
  const handleApprove = () => {
    if (approvedList.length !== 0) {
      ...
      axios
        .put("https://localhost:33123/auth/approve", approvedList, head)
        .then((res) => {
          if (res.data.error) {
          ...
          <Box sx={style}>
            ...
            <div style={{ height: 369, width: 1100 }}>
              <DataGrid
                sx={{
                  m: 2,
                  "& .MuiDataGrid-row:hover": { backgroundColor: "skyblue" },
                }}
            ...
            <button className="buttonito" onClick={handleApprove}>
              Approve Selected
            </button>
          </Box>
        </Modal>
